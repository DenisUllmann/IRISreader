...
class sdo_data_cube:
    """
    This class implements an abstraction of an SDO FITS file collection, 
    ...
    ----------
    files : string
        Path or list of paths to the (sorted) SDO FITS file(s).
    line : string
    ...
    ----------
    type : str
        Observation type: 'sji' or 'raster' or 'aia'.
    obsid : str
    ...
    def __init__( self, files, line='', keep_null=False, force_valid_steps=False ):
        ...
        if 'INSTRUME' not in first_file[0].header.keys() or not first_file[0].header['INSTRUME'][:3] == 'AIA':
            raise CorruptFITSException( "This is not AIA! (according to the INSTRUME header)" )
        ...
        if hasattr( first_file[0], 'shape' ) and len( first_file[0].shape ) == 2:
            ...
            raise CorruptFITSException( "AIA: first extension has only two dimensions (single image, not implemented)" )
    ...
    def _prepare_valid_steps( self ):
        if ir.config.verbosity_level >= 2: print("[sdo_data_cube] Lazy loading valid image steps")
        ...
        if not self._force_valid_steps and os.path.exists( valid_steps_file ):
            if ir.config.verbosity_level >= 2: print("[sdo_data_cube] using precomputed steps")
            try:
    ...
    def _prepare_time_specific_headers( self ): 
        if ir.config.verbosity_level >= 2: print("[sdo_data_cube] Assigning lazy_file_header_list object to time_specific_headers")
    ...
    def _prepare_combined_headers( self ):
        if ir.config.verbosity_level >= 2: print("[sdo_data_cube] Assigning lazy_file_header_list object to headers")
    ...
    def _load_time_specific_header_file( self, file_no ):
        if ir.config.verbosity_level >= 2: print("[sdo_data_cube] Lazy loading time specific headers for file {}".format(file_no))
    ...
    def _load_combined_header_file( self, file_no ):
        raise NotImplementedError( "This feature is not implemented in sdo_data_cube, please use aia_cube" )
         
    # prepare line specific headers
    def _prepare_line_specific_headers( self ):
        if ir.config.verbosity_level >= 2: print("[sdo_data_cube] Lazy loading line specific headers")
    ...
    def get_image_step( self, step ):
        ...
        -------
        numpy.ndarray
            2D image at time step <step>. Format: [y,x] (AIA or SJI), [y,wavelength] (raster).
        """
...
if __name__ == "__main__":
    ...
    fits_data1 = sdo_data_cube( 'C:/Users/Denis/Documents/IRIS/IRIS_SDO_modalities/2014/10/30/aia_l2_20141030_050004_3893010094_94' )
#    fits_data2 = sdo_data_cube( 
